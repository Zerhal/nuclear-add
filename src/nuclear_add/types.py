...
from typing import Union, Callable, Any, Optional, List, Tuple
...
class TracedValue:
    ...
    def __add__(self, other: Union[TracedValue, Any]) -> TracedValue:
        other_val = other.value if isinstance(other, TracedValue) else other
    ...
    def __sub__(self, other: Union[TracedValue, Any]) -> TracedValue:
        other_val = other.value if isinstance(other, TracedValue) else other
        result = self.value - other_val
        return self._record("sub", other, result)
    
    def __mul__(self, other: Union[TracedValue, Any]) -> TracedValue:
        other_val = other.value if isinstance(other, TracedValue) else other
        result = self.value * other_val
        return self._record("mul", other, result)
    
    def __truediv__(self, other: Union[TracedValue, Any]) -> TracedValue:
        other_val = other.value if isinstance(other, TracedValue) else other
        result = self.value / other_val
    ...
    def get_full_trace(self) -> str:
        ...
        for i, entry in enumerate(self.trace):
            lines.append(f"  [{i}] {entry['operands'][0]} {entry['operation']} {entry['operands'][1]} = {entry['result']}")
        return "\n".join(lines)
...
class LazyExpr:
    ...
    def _grad_impl(self, var_name: str) -> LazyExpr:
        ...
        if self.op == "add":
            ...
            if exp.op == "const":
                n = exp.value
                return LazyExpr.const(n) * (base ** LazyExpr.const(n - 1)) * base._grad_impl(var_name)
            # General case: x^y = exp(y * ln(x))
            return self * (exp._grad_impl(var_name) * base.log() + exp * base._grad_impl(var_name) / base)
        elif self.op == "neg":
    ...
    def to_graph(self) -> str:
        ...
        def visit(expr: LazyExpr, parent_id: Optional[str] = None) -> str:
            node_id = f"n{id(expr)}"
...
class StochasticValue:
    ...
    @staticmethod
    def _stochastic_round(value: float, seed: Optional[int] = None) -> float:
        """
        Stochastic rounding.
    ...
    def __add__(self, other: Union[StochasticValue, float]) -> StochasticValue:
...
Numeric = Union[int, float, Decimal, Fraction, complex, Interval, DualNumber, TracedValue, LazyExpr]
