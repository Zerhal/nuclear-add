...
from nuclear_add.types import Interval, DualNumber, TracedValue, LazyExpr, StochasticValue
from nuclear_add.backends import list_available_backends, get_backend
...
def demo_basic_addition():
    ...
    print(f"Nuclear (decimal): add(0.1, 0.2, precision='decimal') = {add(0.1, 0.2, precision='decimal')}")
...
def demo_lazy_evaluation():
    ...
    # Pythagorean: √(x² + y²)
    z = (x * x + y * y).sqrt()
    print(f"Expression: z = √(x² + y²)")
    print(f"Not yet evaluated!")
...
def demo_backends():
    ...
    for name in available:
        ...
        features = []
        if caps.vectorized: features.append("vectorized")
        if caps.gpu: features.append("GPU")
        if caps.jit: features.append("JIT")
        if caps.simd: features.append("SIMD")
        if caps.arbitrary_precision: features.append("arbitrary precision")
        if caps.deterministic: features.append("deterministic")
...
def demo_stochastic_rounding():
    ...
    print(f"Original value: 0.123456789")
    print(f"After stochastic rounding (10 different seeds):")
...
def demo_error_bounds():
    ...
    print(f"After 10 additions of 0.1:")
    print(f"  Interval: {a}")
